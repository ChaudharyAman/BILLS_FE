    ...
    <div className="container mx-auto p-6 font-sans text-slate-900 bg-slate-50/50 min-h-screen">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
        <div>
          <h1 className="text-3xl font-extrabold text-slate-950 flex items-center gap-3">
            <span className="p-2.5 bg-amber-500 text-white rounded-xl shadow-md shadow-amber-100">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
          <span className="text-xs font-bold uppercase text-slate-400">Total Outstanding Debt</span>
          <div className="text-2xl font-black text-slate-900 mt-1">{fmtMoney(totalOutstanding)}</div>
          <span className="text-[11px] text-slate-400 mt-2 block">All active obligations</span>
        </div>

        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
          <span className="text-xs font-bold uppercase text-slate-400">Long-Term Debt</span>
          <div className="text-2xl font-black text-amber-600 mt-1">{fmtMoney(totalLongTerm)}</div>
          <span className="text-[11px] text-slate-400 mt-2 block">Maturity &gt; 12 months</span>
        </div>

        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
          <span className="text-xs font-bold uppercase text-slate-400">Current Portion of LT Debt</span>
          <div className="text-2xl font-black text-indigo-600 mt-1">{fmtMoney(totalCurrentPortion)}</div>
          <span className="text-[11px] text-slate-400 mt-2 block">Principal due in next 12 months</span>
        </div>
      </div>
                ...
                {filteredLiabilities.map((item) => (
                  <tr key={item._id} className="hover:bg-slate-50/70 transition-colors">
                    <td className="py-3.5 px-5">
                      ...
                      <div className="text-[11px] text-slate-400 capitalize">{item.category}</div>
                    </td>
                    <td className="py-3.5 px-4">
                      <span className={`text-[11px] font-bold px-2 py-0.5 rounded-md ${
                        item.type === 'long-term' ? 'bg-purple-50 text-purple-700 border border-purple-200' : 'bg-blue-50 text-blue-700 border border-blue-200'
                      }`}>
                        {item.type}
                      </span>
                    </td>
                    <td className="py-3.5 px-4 text-right font-mono text-xs">{fmtMoney(item.principalAmount)}</td>
                    <td className="py-3.5 px-4 text-right font-mono text-xs font-bold text-slate-900">{fmtMoney(item.outstandingAmount)}</td>
                    <td className="py-3.5 px-4 text-right font-mono text-xs text-indigo-600">
                    ...
                    <td className="py-3.5 px-4 text-center">
                      <span className={`text-[11px] font-bold px-2 py-0.5 rounded-full ${
                        item.status === 'active' ? 'bg-emerald-100 text-emerald-800' : item.status === 'paid' ? 'bg-slate-100 text-slate-600' : 'bg-rose-100 text-rose-800'
                      }`}>
                        {item.status}
                      </span>
                    </td>
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-xs z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-xl border border-slate-200 max-w-lg w-full p-6 relative">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                <label className="block text-xs font-bold text-slate-700 mb-1">Liability Name</label>
                <input
                  type="text"
                  required
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <select
                    ...
                    className="w-full bg-slate-50 border border-slate-300 rounded-xl px-3 py-2 text-xs font-semibold outline-none focus:border-indigo-600 focus:bg-white"
                  >
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-700 mb-1">Outstanding Balance (₹)</label>
                  <input
                    type="number"
                    required
                    min="0"
              ...
              {formData.type === 'long-term' && (
                <div className="bg-indigo-50/70 border border-indigo-200 rounded-xl p-3">
                  ...
                  <input
                    ...
                    className="w-full bg-white border border-indigo-300 rounded-lg px-3 py-1.5 text-xs font-mono outline-none focus:border-indigo-600"
                  />
