...
const PaymentCollection = () => {
  ...
  const getStatusBadge = (diffDays) => {
    if (diffDays === null) {
      return (
        <span className="inline-flex items-center gap-1.5 bg-slate-50 dark:bg-slate-800 text-slate-600 dark:text-slate-300 border border-slate-200 dark:border-slate-700 px-2.5 py-1 rounded-lg text-xs font-semibold">
          Unknown Date
        </span>
      );
    }
    if (diffDays <= 0) {
      return (
        <span className="inline-flex items-center gap-1.5 bg-emerald-50 dark:bg-emerald-950/60 text-emerald-700 dark:text-emerald-300 border border-emerald-150 dark:border-emerald-800/60 px-2.5 py-1 rounded-lg text-xs font-semibold">
    ...
    if (diffDays <= 7) {
      return (
        <span className="inline-flex items-center gap-1.5 bg-amber-50 dark:bg-amber-950/60 text-amber-700 dark:text-amber-300 border border-amber-150 dark:border-amber-800/60 px-2.5 py-1 rounded-lg text-xs font-semibold">
          Overdue by {diffDays} Days
        </span>
      );
    }
    return (
      <span className="inline-flex items-center gap-1.5 bg-red-50 dark:bg-red-950/60 text-red-700 dark:text-red-300 border border-red-150 dark:border-red-800/60 px-2.5 py-1 rounded-lg text-xs font-bold">
        Overdue by {diffDays} Days
      </span>
    );
  };
  ...
  return (
    <div className="container mx-auto p-6 font-sans text-slate-900 dark:text-slate-100 bg-slate-50/50 dark:bg-transparent min-h-screen transition-colors">
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-8 gap-6">
        <div>
          <h1 className="text-4xl font-extrabold text-slate-950 dark:text-slate-100 tracking-tight flex items-center gap-3">
            <span className="p-2.5 bg-gradient-to-tr from-indigo-600 to-indigo-500 rounded-xl text-white shadow-md shadow-indigo-100 dark:shadow-none">
        ...
        {data.invoices.length > 0 && !loading && (
          <div className="w-full lg:w-auto flex flex-col md:flex-row md:items-center gap-3 bg-white dark:bg-slate-900 p-3 rounded-2xl shadow-sm border border-slate-200/80 dark:border-slate-800">
            <div className="flex items-center gap-2.5 px-2 text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">
              <FaDownload className="text-indigo-500 dark:text-indigo-400" /> Export
            </div>
            <ExportDropdown 
      ...
      {/* Info Callout */}
      <div className="flex items-start gap-3 bg-indigo-50/40 dark:bg-indigo-950/40 border border-indigo-100 dark:border-indigo-900/60 rounded-xl p-4 mb-8">
      ...
      <div className="bg-white dark:bg-slate-900 shadow-sm hover:shadow-md transition-shadow duration-300 rounded-2xl border border-slate-200/80 dark:border-slate-800 overflow-hidden mb-12">
        <div className="px-6 py-5 border-b border-slate-100 dark:border-slate-800 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 bg-slate-50/40 dark:bg-slate-800/40">
          <div>
            <h3 className="font-bold text-slate-900 dark:text-slate-100 text-lg">Outstanding Receivables</h3>
            <p className="text-slate-400 dark:text-slate-400 text-xs mt-0.5 font-medium">Detailed queue of clients and invoices pending payment</p>
          </div>
            ...
            <tbody className="bg-white dark:bg-slate-900 divide-y divide-slate-100 dark:divide-slate-800">
              {loading ? (
                ...
                <tr>
                  <td colSpan="5" className="px-6 py-16 text-center">
                    <div className="flex flex-col items-center justify-center max-w-sm mx-auto">
                      <div className="w-12 h-12 bg-emerald-50 dark:bg-emerald-950/60 text-emerald-600 dark:text-emerald-400 rounded-full flex items-center justify-center mb-3.5 border border-emerald-100 dark:border-emerald-800/60">
                        <FaRegClock size={20} />
                      </div>
                      <h4 className="font-bold text-slate-900 dark:text-slate-100 text-base">All Caught Up!</h4>
                ...
                data.invoices.map((row) => (
                  <tr key={row._id} onClick={() => navigate(`/invoices/edit/${row._id}`)} className="hover:bg-slate-50/60 dark:hover:bg-slate-800/60 transition-all duration-150 group cursor-pointer">
                    <td className="px-6 py-4.5 whitespace-nowrap">
                      <div className="text-sm font-bold text-slate-900 dark:text-slate-100 group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors">
                        {row.client?.name || 'Unknown Client'}
                      </div>
                    </td>
                    <td className="px-6 py-4.5 whitespace-nowrap">
                      <div className="text-sm font-semibold text-slate-800 dark:text-slate-200">{row.invoiceNo}</div>
                      <div className="text-[11px] font-semibold text-slate-400 dark:text-slate-400 mt-1 flex items-center gap-1.5">
                    ...
                    <td className="px-6 py-4.5 whitespace-nowrap text-right text-sm font-medium text-slate-500 dark:text-slate-400">
                      ₹{fmt(row.grandTotal)}
                    </td>
                    <td className="px-6 py-4.5 whitespace-nowrap text-right text-sm font-bold text-red-600 dark:text-red-400 bg-red-50/10 dark:bg-red-950/20">
                      ₹{fmt(row.balanceDue)}
                    </td>
                  </tr>
                ))
...
const StatCard = ({ title, amount, color, icon: Icon, subtitle, isCurrency = true }) => {
  const THEME_MAP = {
    indigo: {
      ...
      iconContainer: 'bg-indigo-50 dark:bg-indigo-950/60 text-indigo-600 dark:text-indigo-300 border-indigo-100 dark:border-indigo-800',
      amountText: 'text-indigo-900 dark:text-indigo-200',
    },
    amber: {
      accent: 'bg-amber-500',
      iconContainer: 'bg-amber-50 dark:bg-amber-950/60 text-amber-600 dark:text-amber-300 border-amber-100 dark:border-amber-800',
      amountText: 'text-amber-900 dark:text-amber-200',
    },
  };
