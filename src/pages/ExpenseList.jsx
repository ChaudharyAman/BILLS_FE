...
const ExpenseList = () => {
  ...
  // Salaries Expense (Payroll) states
  ...
  const [payrollTotalRecords, setPayrollTotalRecords] = useState(0);
  ...
  }, [payrollSearchTerm, payrollPage, payrollRowsPerPage, activeTab]);
  ...
  const fetchExpenses = async () => {
    try {
      ...
      params.set('excludeCategoryName', 'Payroll');
  ...
  const monthName = (m) => new Date(0, m - 1).toLocaleString('en-US', { month: 'short' });
      ...
      <div className="bg-white shadow-sm rounded-xl border border-gray-200 overflow-hidden">
        <div className="p-4 border-b border-gray-200 bg-gray-50/50 flex justify-between items-center">
          {activeTab === 'vendor' ? (
            ...
            <input type="text" placeholder="Search payrolls by employee name/ID..."
              className="w-full max-w-sm pl-3 pr-4 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
              value={payrollSearchTerm} onChange={e => { setPayrollSearchTerm(e.target.value); setPayrollPage(1); }} />
          )}
          <div className="text-sm text-gray-500 ml-4">
            Showing {activeTab === 'vendor' ? displayed.length : payrolls.length} of {activeTab === 'vendor' ? totalRecords : payrollTotalRecords}
          </div>
        </div>
            ...
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <th className="px-6 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Status</th>
                  <th className="px-6 py-3 text-right text-xs font-semibold text-gray-500 uppercase tracking-wider">Net Salary</th>
                  <th className="px-6 py-3 text-center text-xs font-semibold text-gray-500 uppercase tracking-wider">Actions</th>
                </tr>
              </thead>
              <tbody className="bg-white divide-y divide-gray-200">
                {payrollLoading ? (
                  ...
                  <tr><td colSpan="6" className="px-6 py-12 text-center text-gray-500 text-sm">No payroll records found.</td></tr>
                ) : payrolls.map(pr => {
                  ...
                  const empId = pr.employeeSnapshot?.employeeId || pr.employee?.employeeId || '—';
                  const designation = pr.employeeSnapshot?.designation || pr.employee?.designation || '';
                  
                  return (
                    <tr key={pr._id} className="hover:bg-blue-50/50 transition-colors">
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 font-semibold">
                        {monthName(pr.month)} {pr.year}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm font-medium text-gray-900">{empName}</div>
                        <div className="text-xs text-gray-500">{empId} {designation ? `· ${designation}` : ''}</div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                        {pr.paymentMethod || 'Bank Transfer'}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className={`px-2.5 py-0.5 rounded-full text-xs font-semibold capitalize border ${PAYROLL_STATUS_STYLES[pr.status] || PAYROLL_STATUS_STYLES.draft}`}>
                          {pr.status}
                        </span>
                      </td>
        ...
        <div className="px-6 py-4 border-t border-gray-200 bg-gray-50 flex flex-col md:flex-row justify-between items-center gap-4">
          <div className="flex items-center gap-2">
            ...
            {activeTab === 'vendor' ? (
              ...
              <select value={payrollRowsPerPage} onChange={e => { setPayrollRowsPerPage(Number(e.target.value)); setPayrollPage(1); }}
                className="border border-gray-300 rounded-md px-2 py-1 text-sm bg-white focus:outline-none focus:ring-1 focus:ring-blue-500">
                <option value={10}>10</option><option value={20}>20</option><option value={50}>50</option><option value={100}>100</option>
              </select>
            )}
          ...
          <div className="flex items-center gap-4">
              <div className="text-sm text-gray-500">
                  Page <span className="font-medium text-gray-900">{activeTab === 'vendor' ? page : payrollPage}</span> of <span className="font-medium text-gray-900">{activeTab === 'vendor' ? (totalPages || 1) : (payrollTotalPages || 1)}</span>
              </div>
              <div className="flex gap-2">
                <button
                  disabled={activeTab === 'vendor' ? page <= 1 : payrollPage <= 1}
                  onClick={() => activeTab === 'vendor' ? setPage(p => p - 1) : setPayrollPage(p => p - 1)}
                  className="px-3 py-1 border border-gray-300 rounded-md text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
                >
                  Previous
                </button>
                <button
                  disabled={activeTab === 'vendor' ? page >= totalPages : payrollPage >= payrollTotalPages}
                  onClick={() => activeTab === 'vendor' ? setPage(p => p + 1) : setPayrollPage(p => p + 1)}
                  className="px-3 py-1 border border-gray-300 rounded-md text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
                >
                  Next
